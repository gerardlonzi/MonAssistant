        ...
        <div className="mt-10 w-full m-auto space-y-1">
          {
            NavLinkEditTemplates.map((el)=>{
                return el.id === getcurrentStep()?.id || StepComplete === true ? 
                    <Link to={el.path} key={el.id} className={clsx(el.id === getcurrentStep()?.id?" bg-[#0040e2]":"bg-transparent","text-white relative flex px-10  text-[15px] font-semibold hover:bg-transparent  py-4 items-center gap-3 w-full  transition-all")}>

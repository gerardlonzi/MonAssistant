  ...
  return (
    type == "password" ?
      ...
      <>
        <div className='flex flex-col w-full'>
          <label className={clsx(required && "after:content-['*'] after:text-red-700 after:text-xl after:ml-[0.5px]",`mb-[1px] uppercase text-sm font-bold text-[#00275b] ${label_className}`)} htmlFor={htmlFor}>{labelName}</label>
          <input onChange={onChange} className={clsx(errors[id] ? "text-red-600 border border-red-600 placeholder:text-red-600 focus:outline-0 " : " border border-[#00275b] focus:outline-0", `rounded-sm  px-2 py-2 ${className} `)} autoComplete='off' disabled={isLoading} type={type} placeholder={placeholder} id={id} {...register(id, {
            required: {
              value: required, message: errorMessage
            }
          })} />
